...
function addCommand(command_name, command_target_array, command_value, auto, insertCommand) {
    ...
    if (s_case) {
	    ...
	    // 은희용 추가
    //console.log("new_record : " + $('tbody#records-grid>tr:last').html());



	/*
	 * Copyright 2017 SideeX committers Capture Plugin
	 *
	 *  Licensed under the Apache License, Version 2.0 (the "License");
	 *  you may not use this file except in compliance with the License.
	 *  You may obtain a copy of the License at
    ...
    var command = $('tbody#records-grid>tr:last>td:nth-child(1)>div:nth-child(1)').text();
    var target = $('tbody#records-grid>tr:last>td:nth-child(2)>div:nth-child(1)').text();
    var value = $('tbody#records-grid>tr:last>td:nth-child(3)>div:nth-child(1)').text();

    registerTestData(command, target, value);
	
}



function registerTestData(command_val, target_val, value_val){
    var formData = new FormData();
    var filename = + new Date();

    //formData.append('ntmKey', ntmKey);
    //formData.append('no', testNo++);
	formData.append('type', "addCommand");
    formData.append('command', command_val);
    formData.append('target', target_val);
    formData.append('value', value_val);
	
	var sendData = "command=" + command_val + "&target=" + target_val + "&value=" + value_val;
	
	/* 은희용 - 사용자가 입력한 메시지를 서버로 전송한다. */
	$.ajax({

		url: 'http://127.0.0.1:42395',
		type: "post",
